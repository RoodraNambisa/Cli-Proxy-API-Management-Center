...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  if (isRecord(remoteManagement)) {
    ...
    const diagnostics = remoteManagement.diagnostics;
    config.remoteManagement = {
      ...
      ...(isRecord(diagnostics)
        ? {
            diagnostics: {
              detailLevel: normalizeString(diagnostics['detail-level'] ?? diagnostics.detailLevel),
            },
          }
        : {}),
    };
    ...
    config.images = {
      ...
      chatgptWeb: chatgptWeb
        ? {
            ...
            autoCleanupLibraryOnFull: normalizeBoolean(
              chatgptWeb['auto-cleanup-library-on-full'] ?? chatgptWeb.autoCleanupLibraryOnFull
            ),
  ...
  if (isRecord(routing)) {
    ...
    config.routingSessionAffinitySubagents = normalizeBoolean(
      Object.prototype.hasOwnProperty.call(routing, 'session-affinity-subagents')
