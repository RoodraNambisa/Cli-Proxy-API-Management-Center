...
test('Claude request retry survives route layers, failed saves, zero and clear reloads', async () => {
  let saved: Array<Record<string, unknown>> = [
    { 'api-key': 'test-original-key', 'request-retry': 5, weight: 7 },
  ];
  ...
  vi.spyOn(useConfigStore.getState(), 'fetchConfig').mockImplementation(
    async () => normalizeConfigResponse({ 'claude-api-key': saved }).claudeApiKeys
  );
  ...
  layer.unmount();
  expect(useClaudeEditDraftStore.getState().drafts['claude:0'].form.requestRetry).toBe(0);
  ...
  expect(useClaudeEditDraftStore.getState().drafts['claude:0'].baseline?.requestRetry).toBe(5);
  ...
  expect(saved[0]).toMatchObject({ 'api-key': 'test-original-key', 'request-retry': 0, weight: 7 });
  first.unmount();
  const second = mount();
