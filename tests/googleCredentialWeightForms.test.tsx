...
describe('Google credential weight forms', () => {
  for (const { section, element } of forms) {
    test(section + ' preserves zero, clear, untouched credentials and a failed draft', async () => {
      let saved: Array<Record<string, unknown>> = [
        { 'api-key': 'test-edited-key', 'base-url': 'https://example.invalid', weight: 5 },
        { 'api-key': 'test-untouched-key', 'base-url': 'https://example.invalid', weight: 17 },
      ];
      ...
      vi.spyOn(useConfigStore.getState(), 'fetchConfig').mockImplementation(async () => {
        ...
        return section === 'vertex-api-key' ? config.vertexApiKeys : config.geminiApiKeys;
      });
      const put = vi.spyOn(apiClient, 'put').mockImplementation(async (_url, body) => { saved = rawCopy(body); return {}; });
      ...
      expect(saved[1]).toMatchObject({ 'api-key': 'test-untouched-key', weight: 17 });
      view.unmount();
      view = mount();
