...
test.each(forms)('$section validates, saves, reloads and clears capacity with model metadata intact', async ({ section, element }) => {
  const copy = (value: unknown) => JSON.parse(JSON.stringify(value));
  const baseModel = { name: 'gemini-test', alias: 'local', 'display-name': 'Label', 'force-mapping': true, future: { keep: true } };
  let saved: Array<Record<string, unknown>> = [{ 'api-key': 'fixture', 'base-url': 'https://example.invalid', weight: 7, models: [{ ...baseModel, 'max-context-length': 131072 }] }];
  ...
  expect(saved[0]).toMatchObject({ 'api-key': 'fixture', weight: 7, models: [{ ...baseModel, 'max-context-length': 1048576 }] });
