...
const blocksNavigation = () => {
  const options = mocks.guard.mock.lastCall?.[0] as unknown as { shouldBlock: (context: unknown) => boolean };
  return options.shouldBlock({ currentLocation: { pathname: '/edit/0' }, nextLocation: { pathname: '/ai-providers' } });
};
function serverFixture() {
  let saved: Array<Record<string, unknown>> = [{ 'api-key': 'test-original-key', 'base-url': 'https://example.invalid', weight: 5 }];
  ...
  const put = vi.spyOn(apiClient, 'put').mockImplementation(async (_url, data) => { saved = JSON.parse(JSON.stringify(data)); return {}; });
  return { put, saved: () => saved };
}

describe('Codex credential weight form', () => {
  test.each([['0', 0], ['', undefined], ['1000000', 1_000_000]] as const)('saves and reloads weight %s without changing the key', async (inputValue, expected) => {
    ...
    expect(server.saved()[0].weight).toBe(expected);
    first.unmount();
    mountForm();
    const reloaded = await screen.findByRole('spinbutton', { name: 'ai_providers.weight_label' });
  ...
  test('invalid and rejected saves preserve the draft and allow a corrected retry', async () => {
    ...
    const input = await screen.findByRole('spinbutton', { name: 'ai_providers.weight_label' });
    await waitFor(() => expect((input as HTMLInputElement).value).toBe('5'));
    for (const value of ['1.5', '1000001', '-1']) {
    ...
    expect(blocksNavigation()).toBe(true);
    expect(server.saved()[0].weight).toBe(5);
    fireEvent.click(screen.getByRole('button', { name: 'common.save' }));
    await screen.findByText('saved');
    expect(server.saved()[0].weight).toBe(0);
  });

  test('a browser bad numeric input is invalid rather than an inherited blank', () => {
    const onChange = vi.fn();
    render(<CredentialWeightInput value={5} onChange={onChange} />);
    const input = screen.getByRole('spinbutton') as HTMLInputElement;
    expect([input.min, input.max, input.step]).toEqual(['0', '1000000', '1']);
    ...
    for (const locale of [en, ru, zhCN, zhTW]) {
      expect(locale.ai_providers.weight_label).toBeTruthy();
      expect(locale.ai_providers.weight_hint).toBeTruthy();
      expect(locale.ai_providers.weight_inherit).toBeTruthy();
      expect(locale.ai_providers.weight_invalid).toBeTruthy();
    }
  });
});
