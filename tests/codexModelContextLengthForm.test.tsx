...
test('Codex capacity edits validate, save, reload and clear without changing aliases or hidden model settings', async () => {
  ...
  const first = mount();
  const field = await screen.findByRole('spinbutton', { name: 'common.model_context_length_label 1' });
  await waitFor(() => expect((field as HTMLInputElement).value).toBe('131072'));
  ...
  fireEvent.change(field, { target: { value: '1048576' } });
  expect(dirty()).toBe(true);
  ...
  expect(saved[0]).toMatchObject({ 'api-key': 'fixture', weight: 3, models: [{ ...model, 'max-context-length': 1048576 }] });
  ...
  expect(dirty()).toBe(false);
  fireEvent.change(reloaded, { target: { value: '0' } });
  expect(dirty()).toBe(true);
  fireEvent.click(screen.getByRole('button', { name: 'common.save' }));
  await screen.findByText('saved');
  expect(saved[0].models).toEqual([{ name: 'gpt-5.5', alias: 'local-model', 'display-name': 'Label', 'force-mapping': true, future: { keep: true } }]);
