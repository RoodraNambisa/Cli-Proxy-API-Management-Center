...
test.each(forms)('$key retains context capacity drafts across layers, rejection, save and reload', async ({ key, section, layout, page, credential }) => {
  const model = { name: 'upstream', alias: 'local', 'display-name': 'Label', thinking: { levels: [] }, 'force-mapping': true, future: { keep: true } };
  let saved: Array<Record<string, unknown>> = [{ ...credential, 'base-url': 'https://example.invalid', models: [{ ...model, 'max-context-length': 131072 }] }];
  ...
  const first = mount();
  const input = await screen.findByRole('spinbutton', { name: 'common.model_context_length_label 1' });
  await waitFor(() => expect((input as HTMLInputElement).value).toBe('131072'));
  ...
  await waitFor(() => expect(within(layer.container).getByLabelText('draft-models').textContent).toContain('1048576'));
  layer.unmount();
  ...
  expect(saved[0]).toMatchObject({ ...credential, models: [{ ...model, 'max-context-length': 1048576 }] });
  first.unmount(); const second = mount();
  await waitFor(() => expect((screen.getByRole('spinbutton', { name: 'common.model_context_length_label 1' }) as HTMLInputElement).value).toBe('1048576'));
  const reloaded = screen.getByRole('spinbutton', { name: 'common.model_context_length_label 1' });
  expect(dirty()).toBe(false);
  ...
  const empty = screen.getByRole('spinbutton', { name: 'common.model_context_length_label 1' });
